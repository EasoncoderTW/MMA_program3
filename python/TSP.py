...
class TravelingSalesmanProblem:
    def __init__(self, coordinates, population_size, generations, calculate_distances=None):
        ...
        self.fitness_function = self.distance  # Default fitness function is the distance
    ...
    def visualize_path(self, path, title, savepath=None, show=False):
        ...
        for i in range(len(path)):
            ...
            plt.text(start[0], start[1], str(path[i]), fontsize=10, color='red' if i % 2 == 0 else 'green')  # Mark index on graph
        plt.title(title)
        plt.grid(True)
